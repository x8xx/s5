...
| `--rx-interface` | (なし) | 受信確認用インターフェース名 |
| `--timeout` | 1000 | 受信タイムアウト（ミリ秒） |

#### 送信のみ
```bash
sudo e_pktgen --interface veth0 --count 10
```

#### 送信＋受信確認
```bash
sudo e_pktgen --interface veth0 --rx-interface veth3 --count 10 --timeout 2000
```

受信確認モードでは、送信したパケットが`--rx-interface`で受信できたかを確認し、結果に応じて終了コードを返します（0: 成功、1: 失敗）。

## 自動テストスクリプト

`test/scripts/`にテストスクリプトを配置しています。
...
| スクリプト | 説明 |
|-----------|------|
| `0001_veth_forwarding.sh` | vethペアを使用した基本的なパケット転送テスト |

### 環境変数

| 変数 | 説明 |
|------|------|
| `DPLANE_BIN` | dplaneバイナリのパス |
| `PKTGEN_BIN` | e_pktgenバイナリのパス |
| `PACKET_COUNT` | 送信パケット数（デフォルト: 10） |
| `TIMEOUT_MS` | 受信タイムアウト（デフォルト: 2000ms） |
